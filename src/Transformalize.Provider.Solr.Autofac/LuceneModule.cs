...
namespace Transformalize.Providers.Solr.Autofac {
    ...
    public class SolrModule : Module {
        ...
        protected override void Load(ContainerBuilder builder) {
            ...
            // Connections
            foreach (var c in process.Connections.Where(cn => cn.Provider == "Solr")) {
                        ...
                        foreach (var analyzer in process.SearchTypes.Where(st => st.Analyzer != string.Empty && st.Analyzer != DefaultAnalyzer).Select(st => st.Analyzer).Distinct()) {
                            switch (analyzer) {
                                ...
                                case "whitespace":
                                    ...
                                    break;
                                case "standard":
                                    builder.Register<Analyzer>(ctx => new global::Solr.Net.Analysis.Standard.StandardAnalyzer(global::Solr.Net.Util.Version.Solr_30)).Named<Analyzer>(analyzer);
                                    break;
                                default:
            ...
            foreach (var entity in process.Entities.Where(e => process.Connections.First(c => c.Name == e.Connection).Provider == "Solr")) {
                ...
                builder.Register<IInputProvider>(ctx => {
                    ...
                    switch (input.Connection.Provider) {
                        case "Solr":
                            return new SolrInputProvider(input, ctx.ResolveNamed<SearcherFactory>(entity.Key));
                        default:
                ...
                builder.Register<IRead>(ctx => {
                    ...
                    switch (input.Connection.Provider) {
                        case "Solr":
                            return new SolrReader(input, input.InputFields, ctx.ResolveNamed<SearcherFactory>(entity.Key), ctx.ResolveNamed<Analyzer>(entity.Key + ReadFrom.Input), ctx.ResolveNamed<IndexReaderFactory>(entity.Key), rowFactory, ReadFrom.Input);
                        default:
            ...
            if (process.Output().Provider == "Solr") {
                ...
                foreach (var entity in process.Entities) {
                        ...
                        switch (output.Connection.Provider) {
                            case "Solr":
                                return new SolrOutputController(
                                    output,
                                    new SolrInitializer(output), 
                                    ctx.ResolveNamed<IInputProvider>(entity.Key),
                                    new SolrOutputProvider(output, ctx.ResolveNamed<SearcherFactory>(entity.Key)),
                    ...
                    builder.Register<IWrite>(ctx => {
                        ...
                        switch (output.Connection.Provider) {
                            case "Solr":
                                return new SolrWriter(output, ctx.ResolveNamed<IndexWriterFactory>(entity.Key), ctx.ResolveNamed<SearcherFactory>(entity.Key));
                            default:
                        ...
                        builder.Register<IEntityDeleteHandler>(ctx => {
                            ...
                            switch (outputConnection.Provider) {
                                case "Solr":
                                    ...
                                    //TODO: need SolrUpdater (update TflDeleted to true)
                                    break;
                            }
